...
### Targeted User Flow
Engagement can be thought of as a "funnel", both in shape and process. Most broadly, there is general traffic on a site. As users interact with  sites, with some subtle prodding they move further down the funnel toward the goal of a campaign, in our case to sign up and follow. However, at any point in the flow users can _disengage_. Attrition occurs at every step in the funnel as it is highly unlikely that all users will behave as we want. At the end of the funnel is the conversion point, where we can say that a user starting at point A found themselves at point B and not by coincidence. The targeted user flow, or funnel, for our scenario is described below.
